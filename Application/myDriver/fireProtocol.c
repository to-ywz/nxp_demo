...
#include "fireProtocol.h"
#include <string.h>
#include "control_pid.h"
#include "PID.h"

extern pid_param_t PID_R;
extern pid_param_t PID_L;
extern pid_param_t PID_R_L_Blance;
...
int8_t receiving_process(void)
{
  ...
  while (1)
  {
    ...
    case SET_P_I_D_CMD:
    {
      ...
      PidSetValue(&PID_R, p_temp, i_temp, d_temp); // 设置 P I D
    }
    break;
    ...
    case SET_TARGET_CMD:
    {
      ...
      MotorR.expectedSpeed = actual_temp; // 设置目标值
    }
    break;
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  UART_PutBuff(LPUART1, (uint8_t *)&set_packet, sizeof(set_packet)); // 发送数据头
  UART_PutBuff(LPUART1, (uint8_t *)data, num);                       // 发送参数
  UART_PutBuff(LPUART1, (uint8_t *)&sum, sizeof(sum));               // 发送校验和
}
