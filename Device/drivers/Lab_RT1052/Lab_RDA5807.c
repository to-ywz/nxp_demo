...
/**
  ...
  * @note     无
  *
  * @example  RDA_Delayms(100);
  *
  * @date     2019/4/22 星期一
*/
void RDA_Delayms(uint16_t ms)
{
...
{
  char txt[16];
  unsigned char  Vol=10,RSSI=0,mute=1;
...
{
  unsigned short  i = 0;
  for(i = 0; i <50000; i++) //修改这里可以调整FMI2C速率
  {
      __asm("NOP"); /* delay */
  }
}
...
*  函数返回：无
*  修改时间：2020年3月10日
*  应用举例：FMI2C_Init();  //模拟FMI2C初始化 FMI2C管脚在Lab_SOFTI2C.h中定义
*************************************************************************/
