...
/**
  ...
  * @date     2019/5/23 星期四
  */
void Lab_PWM_Init(PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwm_channels, uint16_t Frequency)
{

    //管脚复用选择
    Lab_PWM_InitPins(base, subModule, pwm_channels);
...
/**
  ...
  * @date     2019/5/23 星期四
  */
inline void Lab_PWM_SetDuty(PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwm_channels, uint16_t duty)
{
 ...
 * @brief           设置 舵机 PWM
 * 
 * @param channel   PWM模块下的子模块 的通道
 *                      kPWM_PwmA, kPWM_PwmB, kPWM_PwmA_B
 *                      0: kPWMA_PwmA   M3  // 
 *                      1: kPWMA_PwmB   M4
 * @param duty      duty * DUTY_MAX
 * 
 * @date 2021-05-27
 */
inline void Lab_ServoCtrl(uint8_t channel, uint16_t duty)
{
    if (!channel)
    {
        Lab_PWM_SetDuty(PWM2, kPWM_Module_3, kPWM_PwmA, duty);
    }
    else
    {
        Lab_PWM_SetDuty(PWM2, kPWM_Module_3, kPWM_PwmB, duty);
    }
}
...
/**
  ...
  * @date     2019/5/23 星期四
  */
inline void Lab_PWMA_B_SetDuty(PWM_Type *base, pwm_submodule_t subModule, uint16_t dutyA, uint16_t dutyB)
{
...
/**
  ...
  * @date     2019/5/23 星期四
  */
inline void Lab_SetServoDty(uint16_t duty)
{
    if (duty > 850)
        duty = 850; //限幅，可自行修改
    if (duty < 650)
        duty = 650;
    Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_3, duty, duty); //M3 M4
}
...
/**
  ...
  * @date     2019/5/23 星期四
  */
inline void Lab_SetMotorDty(uint8_t num, short duty)
{
    assert(num < 5);
    ...
    case 1:
        if (duty >= 0)
        {
            Lab_PWMA_B_SetDuty(PWM1, kPWM_Module_3, 0, duty);
        }
        else
        {
            Lab_PWMA_B_SetDuty(PWM1, kPWM_Module_3, -duty, 0);
        }
        break;
    ...
    case 2:
        if (duty >= 0)
        {
            Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_0, 0, duty);
        }
        else
        {
            Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_0, -duty, 0);
        }
        break;
    ...
    case 3:
        if (duty >= 0)
        {
            Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_1, 0, duty);
        }
        else
        {
            Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_1, -duty, 0);
        }
        break;
    ...
    case 4:
        if (duty >= 0)
        {
            Lab_PWM_SetDuty(PWM2, kPWM_Module_2, kPWM_PwmB, 0);
            Lab_PWM_SetDuty(PWM1, kPWM_Module_1, kPWM_PwmA, duty);
        }
        else
        {
            Lab_PWM_SetDuty(PWM2, kPWM_Module_2, kPWM_PwmB, -duty);
            Lab_PWM_SetDuty(PWM1, kPWM_Module_1, kPWM_PwmA, 0);
        }
        break;
    }
}
...
void Test_Servo(void)
{
    ...
    Lab_PWM_Init(PWM2, kPWM_Module_3, kPWM_PwmA_B, 50); //M3 M4

    Lab_SetServoDty(servopwm); //中值

    TFTSPI_P8X16Str(1, 0, "Lab Servo PWM", u16RED, u16BLUE);
    ...
    while (1)
    {
        ...
        case 1:
            ...
            Lab_SetServoDty(servopwm); //刷新servopwm频率
            break;
        case 2:
            servopwm = 750;
            Lab_SetServoDty(servopwm); //刷新servopwm频率
...
void Test_Motor(void)
{
    ...
    LED_Init();
    /*初始化电机PWM接口   */
    Lab_PWM_Init(PWM2, kPWM_Module_0, kPWM_PwmA_B, 18000); //  A8 A9
    Lab_PWM_Init(PWM2, kPWM_Module_1, kPWM_PwmA_B, 18000); //  B9 C9
    Lab_PWM_Init(PWM2, kPWM_Module_2, kPWM_PwmB, 18000);   //  A10
    Lab_PWM_Init(PWM1, kPWM_Module_1, kPWM_PwmA, 18000);   //  J1
    Lab_PWM_Init(PWM1, kPWM_Module_3, kPWM_PwmA_B, 18000); //  L5 M5
    TFTSPI_P8X16Str(3, 0, "Lab Motor PWM", u16RED, u16BLUE);
    ...
    while (1)
    {
        ...
        case 1:
            ...
            if (motorpwm < -10000)
                motorpwm = -10000;
            Lab_SetMotorDty(1, motorpwm);
            Lab_SetMotorDty(2, motorpwm);
            Lab_SetMotorDty(3, motorpwm);
            Lab_SetMotorDty(4, motorpwm);
            break;
        case 3:
            motorpwm += 100;
            if (motorpwm > 10000)
...
void Test_Servo(void)
{
    ...
    LCD_P8x16Str(13, 0, "Lab Servo PWM");
...
void Test_Motor(void)
{
    ...
    LCD_CLS(); //LCD清屏
    /*初始化电机PWM接口   */
    Lab_PWM_Init(PWM2, kPWM_Module_0, kPWM_PwmA_B, 12000); // A8 A9
    Lab_PWM_Init(PWM2, kPWM_Module_1, kPWM_PwmA_B, 12000); // B9 C9
    Lab_PWM_Init(PWM2, kPWM_Module_2, kPWM_PwmB, 12000);   // A10
    Lab_PWM_Init(PWM1, kPWM_Module_1, kPWM_PwmA, 12000);   // J1
    Lab_PWM_Init(PWM1, kPWM_Module_3, kPWM_PwmA_B, 12000); // L5 M5
    LCD_P8x16Str(13, 0, "Lab Motor PWM");
...
void Test_PWM_nCH(void)
{
    ...
    LED_Init();
    /*初始化电机PWM接口  */
    Lab_PWM_Init(PWM2, kPWM_Module_0, kPWM_PwmA_B, 100);   // A8 A9
    Lab_PWM_Init(PWM2, kPWM_Module_1, kPWM_PwmA_B, 1000);  // B9 C9
    Lab_PWM_Init(PWM2, kPWM_Module_2, kPWM_PwmB, 10000);   // A10
    Lab_PWM_Init(PWM1, kPWM_Module_3, kPWM_PwmA_B, 20000); // L5 M5
    Lab_PWM_Init(PWM1, kPWM_Module_1, kPWM_PwmA, 12000);   // J1
    Lab_PWM_Init(PWM2, kPWM_Module_3, kPWM_PwmA_B, 50);    // M3 M4
    ...
    while (1U)
    {
        ...
        Lab_PWM_SetDuty(PWM1, kPWM_Module_1, kPWM_PwmA, pwmVal * 50);
        Lab_PWM_SetDuty(PWM1, kPWM_Module_3, kPWM_PwmA, pwmVal * 50);
        Lab_PWM_SetDuty(PWM1, kPWM_Module_3, kPWM_PwmB, pwmVal * 100);
        Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_0, pwmVal * 100, pwmVal * 50);
        Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_1, pwmVal * 100, pwmVal * 50);
        Lab_PWM_SetDuty(PWM2, kPWM_Module_2, kPWM_PwmB, pwmVal * 100);
        Lab_PWMA_B_SetDuty(PWM2, kPWM_Module_3, pwmVal * 100, pwmVal * 50);
...
/**
  ...
  * @date     2019/5/23 星期四
  */
void Lab_PWM_InitPins(PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwm_channels)
{
