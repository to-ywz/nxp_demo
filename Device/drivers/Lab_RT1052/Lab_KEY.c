...
{
    /* 设置上拉输入 */
    Lab_PinInit(P2, PIN_MODE_INPUT_PULLUP, 1);
    Lab_PinInit(C13, PIN_MODE_INPUT_PULLUP, 1);
    Lab_PinInit(C14, PIN_MODE_INPUT_PULLUP, 1);
}
...
uint8_t KEY_Read(uint8_t mode)
{
    ...
    if (key_up && (Lab_PinRead(C13) == 0 || Lab_PinRead(C14) == 0 || Lab_PinRead(P2) == 0))
    {
        delayms(10); //消抖
        key_up = 0;
        if (Lab_PinRead(C13) == 0)
        {
            return 1;
        }

        else if (Lab_PinRead(C14) == 0)
        {
            return 2;
        }

        else if (Lab_PinRead(P2) == 0)
        {
            return 3;
        }
    }
    if (Lab_PinRead(C13) == 1 && Lab_PinRead(C14) == 1 && Lab_PinRead(P2) == 1)
    {
        key_up = 1;
    }
    return 0; //无按键按下
...
void Test_ExtiKEY(void)
{
    ...
    /* 配置下降沿触发中断 */
    Lab_PinExti(C13, PIN_IRQ_MODE_FALLING);
    Lab_PinExti(C14, PIN_IRQ_MODE_FALLING);
    Lab_PinExti(P2, PIN_IRQ_MODE_FALLING);
